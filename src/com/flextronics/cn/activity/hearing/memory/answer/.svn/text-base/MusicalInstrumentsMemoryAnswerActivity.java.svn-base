...
public class MusicalInstrumentsMemoryAnswerActivity extends BaseActivity {
	private static final String TAG = "MusicalInstrumentsAnswerActivity";
	...
	private void setupView() {
		if (elementType.equals(HearingConstants.Element.FOREIGN_MUSIC_SCALE)) {
			setTrainingTitle(getString(R.string.hearing_system_memory_training_musical_instruments_foreign_music));
		} else if (elementType.equals(HearingConstants.Element.FOLK_MUSIC_SCALE)) {
			setTrainingTitle(getString(R.string.hearing_system_memory_training_musical_instruments_folk_music));
		} else if (elementType.equals(HearingConstants.Element.PERCUSSION_SCALE)) {
			setTrainingTitle(getString(R.string.hearing_system_memory_training_musical_instruments_percussion));
		}
	...
	private void setupListener() {
		answer.setOnItemClickListener(new OnItemClickListener() {
			public void onItemClick(AdapterView<?> parent, View view,
				...
				musicalInstruments=musicalInstrumentsSet.get(position);
				onClick(musicalInstruments);
			}
		});
		...
		setOnBackButtonTouchListener(new OnTouchListener() {
			public boolean onTouch(View v, MotionEvent event) {
				if (event.getAction() == MotionEvent.ACTION_UP) {
					Intent intent = new Intent(getApplication(),MusicalInstrumentsMemoryActivity.class);
	...
	private void showTestReport(final MemoryReport testReport) {
		new AlertDialog.Builder(MusicalInstrumentsMemoryAnswerActivity.this).setTitle(R.string.tips)
						...
						new DialogInterface.OnClickListener() {
							public void onClick(DialogInterface dialog,
								...
								if (bitType.equals(HearingConstants.BitType.CONTINUED_BIT)) {
									...
									Intent intent = new Intent(MusicalInstrumentsMemoryAnswerActivity.this, TestReportWithTableActivity.class);
									...
									Intent intent = new Intent(MusicalInstrumentsMemoryAnswerActivity.this, TestReportActivity.class);
									bundle.putSerializable(Constants.TEST_REPORT, testReport);
									bundle.putString(Constants.CLASS_NAME, MusicalInstrumentsMemoryAnswerActivity.class.getName());
									bundle.putString(HearingConstants.PREVIOUS_ACTIVITY, MusicalInstrumentsMemoryActivity.class.getName());
									intent.putExtras(bundle);
									startActivity(intent);	
								}
								finish();		
	...
	private void setDisplayAnswer(){
		...
		if (randomMusicalInstruments) {
			musicalInstrumentsSet.clear();
			int[] temp=HearingUtil.DataConversion.stringArrayToInteger(question.getDisplayContents());
			musicalInstrumentsSet=HearingUtil.DataConversion.integerArrayToList(temp);
			musicalInstrumentsAdapter=
			new MusicalInstrumentsAdapter(getApplicationContext(), true, temp);
			answer.setAdapter(musicalInstrumentsAdapter);
		}else {
			musicalInstrumentsAdapter=
			new MusicalInstrumentsAdapter(getApplicationContext(),true, HearingUtil.DataConversion.listToInteger(musicalInstrumentsSet));
			answer.setAdapter(musicalInstrumentsAdapter);
		}
	}
	...
	private void setDisplayAnswerText(int answer){
		if (answer!=0) {
			String[] temp=getResources().getStringArray(R.array.MusicalInstrumentsNameArray);
			answerTextString+=temp[answer-100]+" ";
	...
	private void showRightAnswer(){
		...
		String[] tempStrings=getResources().getStringArray(R.array.MusicalInstrumentsNameArray);
		for (int i = 0; i < question.getAnswers().length; i++) {
			tempString+=tempStrings[Integer.parseInt(question.getAnswers()[i])-100]+" ";
		}
	...
	private void playQuestionMusic(String questionContent) {
		...
		try {
			if (!file.exists()) {
				...
				InputStream inputStream = MusicalInstrumentsMemoryAnswerActivity.this.getAssets()
			...
			if (errorCount <= 25) {
				...
				new AlertDialog.Builder(MusicalInstrumentsMemoryAnswerActivity.this).setTitle(R.string.tips).setMessage(R.string.internal_error)
						.setPositiveButton(R.string.ok,
								new DialogInterface.OnClickListener() {
									public void onClick(DialogInterface dialog,
											int which) {
										MusicalInstrumentsMemoryAnswerActivity.this.finish();
									}
								}).show();
				errorCount = 0;
			}
