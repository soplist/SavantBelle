...
public class MusicalInstrumentsResponseActivity extends BaseActivity {	
		...
		setOnBackButtonTouchListener(new OnTouchListener() {
			public boolean onTouch(View v, MotionEvent event) {
				if (event.getAction() == MotionEvent.ACTION_UP) {
					startActivity(
					new Intent(getApplicationContext(),HearingActivity.class));
					finish();
				}
				return false;
			}
		...
		setOnOkButtonTouchListener(new OnTouchListener() {
			public boolean onTouch(View v, MotionEvent event) {
				if (event.getAction() == MotionEvent.ACTION_UP) {
					...
					intent.setClass(getApplicationContext(), MusicalInstrumentsResponseAnswerActivity.class);
		...
		elementPanel.findViewById(HearingConstants.ViewId.VIEW_ID_ONE).setOnTouchListener(new OnTouchListener() {
			public boolean onTouch(View v, MotionEvent event) {
				...
				if (event.getAction()==MotionEvent.ACTION_UP) {
					...
					setElementPanelParams(musicalInstruments, musicalInstrumentsPanel, scale, scalePanel, elementType, true);
				}
				return false;
			}
		});
		...
		elementPanel.findViewById(HearingConstants.ViewId.VIEW_ID_THREE).setOnTouchListener(new OnTouchListener() {
			public boolean onTouch(View v, MotionEvent event) {
				...
				if (event.getAction()==MotionEvent.ACTION_UP) {
					...
					elementText=((Button)v).getText().toString();
					setDeputyTitle(element, elementText);
					setElementPanelParams(musicalInstruments, musicalInstrumentsPanel, scale, scalePanel, elementType, false);
				}
				return false;
			}
		});
		...
		musicalInstrumentsPanel.findViewById(HearingConstants.ViewId.VIEW_ID_TWO).setOnTouchListener(new OnTouchListener() {
			public boolean onTouch(View v, MotionEvent event) {
				...
				if (event.getAction()==MotionEvent.ACTION_UP) {
					...
					randomEnabled=false;
					clearData(musicalInstrumentsSet);
					optionManyMusicalInstruments(elementType);
				}
				return false;
			}
		});
	...
	private void setElementPanelParams(View view,View view2,View view3,View view4,String string,boolean state){
		randomEnabled=false;
		clearData(musicalInstrumentsSet);
		setManyMusicalInstruments(string);
		setDeputyTitle(view, 
	...
	private void setManyMusicalInstruments(String params){
		if (params.equals(HearingConstants.Element.FOREIGN_MUSIC_SCALE)) {
			musicalInstrumentsSet.add(MusicalInstruments.ForeignMusicScale.PIANO);
			musicalInstrumentsSet.add(MusicalInstruments.ForeignMusicScale.TRUMPET);
			musicalInstrumentsSet.add(MusicalInstruments.ForeignMusicScale.VIOLIN);
		}else if (params.equals(HearingConstants.Element.FOLK_MUSIC_SCALE)) {
			musicalInstrumentsSet.add(MusicalInstruments.FolkMusicScale.PI_PA);
			musicalInstrumentsSet.add(MusicalInstruments.FolkMusicScale.ER_HU);
			musicalInstrumentsSet.add(MusicalInstruments.FolkMusicScale.SUO_NA);
		}else if (params.equals(HearingConstants.Element.PERCUSSION_SCALE)) {
			musicalInstrumentsSet.add(MusicalInstruments.PercussionScale.KICK);
			musicalInstrumentsSet.add(MusicalInstruments.PercussionScale.WOODBLOCK);
			musicalInstrumentsSet.add(MusicalInstruments.PercussionScale.TRIANGLE);
		}else {
			
		}
	}
	
	private void optionManyMusicalInstruments(final String params){
		int[] temp=null;
		if (params.equals(HearingConstants.Element.FOREIGN_MUSIC_SCALE)) {
			temp=MusicalInstruments.ForeignMusicScale.ARRAY;
		}else if (params.equals(HearingConstants.Element.FOLK_MUSIC_SCALE)) {
			temp=MusicalInstruments.FolkMusicScale.ARRAY;
		}else if (params.equals(HearingConstants.Element.PERCUSSION_SCALE)) {
			temp=MusicalInstruments.PercussionScale.ARRAY;
		}
		final int[] temp2=temp;
		RelativeLayout relativeLayout = new RelativeLayout(getApplicationContext());
		final Dialog dialog = new Dialog(this, R.style.dialog);
		Gallery gallery = new Gallery(getApplicationContext());
		MusicalInstrumentsAdapter musicalInstrumentsAdapter=new MusicalInstrumentsAdapter(this,false,temp2);
		...
		gallery.setOnItemClickListener(new OnItemClickListener() {
			public void onItemClick(AdapterView<?> parent, View view,int position, long id) {
				...
				if (musicalInstrumentsSet.size() == HearingConstants.DEFAULT_ELEMENT_COUNT_MEDIUM) {
					musicalInstrumentsNameText=nameArray(HearingConstants.Smaple.MUSICAL_INSTRUMENTS, musicalInstrumentsSet);
					setDeputyTitle(musicalInstruments, musicalInstrumentsText+"¦"+musicalInstrumentsNameText);
					dialog.cancel();
				}
	...
	private void optionSingleScale(){
		RelativeLayout relativeLayout = new RelativeLayout(getApplicationContext());
		final Dialog dialog = new Dialog(this, R.style.dialog);
		Gallery gallery = new Gallery(getApplicationContext());
		...
		gallery.setOnItemClickListener(new OnItemClickListener() {
			public void onItemClick(AdapterView<?> parent, View view,int position, long id) {
				...
				if (scaleSet.size() == HearingConstants.DEFAULT_ELEMENT_COUNT_LOW) {
					scaleNameText=nameArray(HearingConstants.Smaple.SCALE, scaleSet);
					setDeputyTitle(scale, scaleText+"¦"+scaleNameText);
					dialog.cancel();
				}
	...
	private void randomScale(){
		scaleSet.add(ArrayOperations.getRedomElementFromElements(HearingConstants.Scale.ARRAY));
	}
	...
	private void choosedScalePanel(){
		...
		Button button2=new Button(this);
		button2.setId(HearingConstants.ViewId.VIEW_ID_TWO);
		button2.setText(getString(R.string.hearing_system_optional_single_scale));
		button2.setTextSize(HearingConstants.TEXT_SIZE_SMALL);
		button2.setBackgroundDrawable(getResources().getDrawable(R.drawable.cs_choose_params_btn1_1));
		button2.setGravity(Gravity.CENTER);
		button2.setClickable(true);
		...
		Button button3=new Button(this);
		button3.setId(HearingConstants.ViewId.VIEW_ID_THREE);
		button3.setText(getString(R.string.hearing_system_random_scale));
		button3.setTextSize(HearingConstants.TEXT_SIZE_SMALL);
		button3.setBackgroundDrawable(getResources().getDrawable(R.drawable.cs_choose_params_btn1_1));
		button3.setGravity(Gravity.CENTER);
		button3.setClickable(true);
