...
public class ScaleResponseActivity extends BaseActivity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setSingleMusicalInstruments(elementType);
		setManyScale();
		...
		scaleText=getString(R.string.hearing_system_set_many_scale);
		...
		setOnOkButtonTouchListener(new OnTouchListener() {
			public boolean onTouch(View v, MotionEvent event) {
				if (event.getAction() == MotionEvent.ACTION_UP) {
					...
					intent.setClass(getApplicationContext(), ScaleResponseAnswerActivity.class);
		...
		musicalInstrumentsPanel.findViewById(HearingConstants.ViewId.VIEW_ID_TWO).setOnTouchListener(new OnTouchListener() {
			public boolean onTouch(View v, MotionEvent event) {
				...
				if (event.getAction()==MotionEvent.ACTION_UP) {
					...
					musicalInstrumentsText=((Button)v).getText().toString();
					clearData(musicalInstrumentsSet);
					optionSingleMusicalInstruments(elementType);
				}
				return false;
			}
		});
		...
		musicalInstrumentsPanel.findViewById(HearingConstants.ViewId.VIEW_ID_THREE).setOnTouchListener(new OnTouchListener() {
			public boolean onTouch(View v, MotionEvent event) {
				...
				if (event.getAction()==MotionEvent.ACTION_UP) {
					...
					musicalInstrumentsText=((Button)v).getText().toString();
					clearData(musicalInstrumentsSet);
					randomMusicalInstruments(elementType);
		...
		scalePanel.findViewById(HearingConstants.ViewId.VIEW_ID_ONE).setOnTouchListener(new OnTouchListener() {
			public boolean onTouch(View v, MotionEvent event) {
				...
				if (event.getAction()==MotionEvent.ACTION_UP) {
					...
					clearData(scaleSet);
					setManyScale();
					scaleNameText=nameArray(HearingConstants.Smaple.SCALE, musicalInstrumentsSet);
	...
	private void setElementPanelParams(View view,View view2,View view3,View view4,String string){
		randomEnabled=false;
		clearData(musicalInstrumentsSet);
		setSingleMusicalInstruments(string);
		setDeputyTitle(view, 
	...
	private void setSingleMusicalInstruments(String params){
	...
	private void optionSingleMusicalInstruments(final String params){
		int[] temp=null;
		if (params.equals(HearingConstants.Element.FOREIGN_MUSIC_SCALE)) {
		...
		gallery.setOnItemClickListener(new OnItemClickListener() {
			public void onItemClick(AdapterView<?> parent, View view,int position, long id) {
				musicalInstrumentsSet.add(temp2[position]);
	...
	private void optionManyScale(){
		...
		gallery.setOnItemClickListener(new OnItemClickListener() {
			public void onItemClick(AdapterView<?> parent, View view,int position, long id) {
				checkList(HearingConstants.Scale.ARRAY[position], scaleSet);
	...
	private void randomMusicalInstruments(String params){
		if (params.equals(HearingConstants.Element.FOREIGN_MUSIC_SCALE)) {
			musicalInstrumentsSet.add(ArrayOperations.getRedomElementFromElements(MusicalInstruments.ForeignMusicScale.ARRAY));
		}else if (params.equals(HearingConstants.Element.FOLK_MUSIC_SCALE)) {
			musicalInstrumentsSet.add(ArrayOperations.getRedomElementFromElements(MusicalInstruments.FolkMusicScale.ARRAY));
		}
	}
	...
	private void choosedScalePanel(){
		...
		Button button=new Button(this);
		button.setId(HearingConstants.ViewId.VIEW_ID_ONE);
		button.setText(getString(R.string.hearing_system_set_many_scale));
		button.setTextSize(HearingConstants.TEXT_SIZE_SMALL);
		button.setBackgroundDrawable(getResources().getDrawable(R.drawable.cs_choose_params_btn1_1));
		button.setGravity(Gravity.CENTER);
		button.setClickable(true);
		...
		Button button2=new Button(this);
		button2.setId(HearingConstants.ViewId.VIEW_ID_TWO);
		button2.setText(getString(R.string.hearing_system_optional_many_scale));
		button2.setTextSize(HearingConstants.TEXT_SIZE_SMALL);
		button2.setBackgroundDrawable(getResources().getDrawable(R.drawable.cs_choose_params_btn1_1));
		button2.setGravity(Gravity.CENTER);
		button2.setClickable(true);
