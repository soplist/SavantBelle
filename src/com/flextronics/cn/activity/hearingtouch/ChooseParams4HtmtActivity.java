 ...
 *
 */
public class ChooseParams4HtmtActivity extends BaseActivity {
			...
			public boolean onTouch(View v, MotionEvent event) {
				if (event.getAction() == MotionEvent.ACTION_UP) {					
					...
					if (testingTypeLinearLayoutBtnGroup.getValue().equals(MUSIC)) {
						intent.setClass(getApplicationContext(), ChooseParams4HtmtMusicActivity.class);					
					}else if (testingTypeLinearLayoutBtnGroup.getValue().equals(SCALE)) {
						intent.setClass(getApplicationContext(), ChooseParams4HtmtScaleActivity.class);
					}
