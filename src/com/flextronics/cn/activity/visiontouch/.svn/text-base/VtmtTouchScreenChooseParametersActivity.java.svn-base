...
public class VtmtTouchScreenChooseParametersActivity extends BaseActivity {
	public static final String TAG="VtmtTouchScreenChooseParametersActivity";
	...
	private RelativeLayout responsePanel;
	...
	private String responseTypeText="样本位置出现顺序回应";
	...
	private int responseTypeParameters=Constants.VisioTouchMemoryTrainingUIParameter.SAMPLE_LOCATION;
	...
	private void setupView(){
		...
		setButtonOnTouchUpBackground(responsePanel, HearingConstants.ViewId.VIEW_ID_ONE);
	...
	private void setupListener(){
		...
		setOnBackButtonTouchListener(new OnTouchListener() {
			public boolean onTouch(View v, MotionEvent event) {
				if (event.getAction()==MotionEvent.ACTION_UP) {
					startActivity(new Intent(VtmtTouchScreenChooseParametersActivity.this, VhtChooseTrainingActivity.class));
					finish();
				}
				return false;
			}
		...
		setOnOkButtonTouchListener(new OnTouchListener() {
			public boolean onTouch(View v, MotionEvent event) {
				if (event.getAction()==MotionEvent.ACTION_UP) {
					...
					intent.setClass(VtmtTouchScreenChooseParametersActivity.this,VtmtTouchScreenOperationActivity.class);
					...
					bundle.putInt(Constants.VisioTouchMemoryTrainingUIParameter.PRESENT_TYPE,responseTypeParameters);
		...
		setResponseTypeAndResponseTypePanelListener();
	...
	private void setResponseTypeAndResponseTypePanelListener(){
		response.setOnTouchListener(new OnTouchListener() {
			public boolean onTouch(View v, MotionEvent event) {
				if (event.getAction()==MotionEvent.ACTION_UP) {
					if (responsePanel.getVisibility()==LinearLayout.VISIBLE) {
						v.setBackgroundDrawable(getResources().getDrawable(R.drawable.ui_params_btn_normal));
						responsePanel.setVisibility(LinearLayout.GONE);
					}else {
						v.setBackgroundDrawable(getResources().getDrawable(R.drawable.ui_params_btn_select));
						responsePanel.setVisibility(LinearLayout.VISIBLE);
					}
				}
				return false;
			}
		...
		responsePanel.findViewById(HearingConstants.ViewId.VIEW_ID_ONE).setOnTouchListener(new OnTouchListener() {
			public boolean onTouch(View v, MotionEvent event) {
				if (event.getAction()==MotionEvent.ACTION_DOWN) {
					Log.d(TAG, "true");
					setButtonOnTouchDownBackground(responsePanel, HearingConstants.ViewId.VIEW_ID_ONE);
				}
				
				if (event.getAction()==MotionEvent.ACTION_UP) {
					setButtonOnTouchUpBackground(responsePanel, HearingConstants.ViewId.VIEW_ID_ONE);
					setDeputyTitle(response, ((Button)v).getText().toString());
				}
				
				responseTypeParameters=Constants.VisioTouchMemoryTrainingUIParameter.SAMPLE_LOCATION;
				return false;
			}
		});
		
		responsePanel.findViewById(HearingConstants.ViewId.VIEW_ID_TWO).setOnTouchListener(new OnTouchListener() {
			public boolean onTouch(View v, MotionEvent event) {
				if (event.getAction()==MotionEvent.ACTION_DOWN) {
					Log.d(TAG, "true");
					setButtonOnTouchDownBackground(responsePanel, HearingConstants.ViewId.VIEW_ID_TWO);
				}
				
				if (event.getAction()==MotionEvent.ACTION_UP) {
					setButtonOnTouchUpBackground(responsePanel, HearingConstants.ViewId.VIEW_ID_TWO);
					setDeputyTitle(response, ((Button)v).getText().toString());
				}
				
				responseTypeParameters=Constants.VisioTouchMemoryTrainingUIParameter.SAMPLE_ELEMENT;
				return false;
			}
		});
	}
	...
	private void showChooseBitDialog(int from, int to, int x, int y, final String type){
		...
		final Dialog dialog = new Dialog(VtmtTouchScreenChooseParametersActivity.this, R.style.cs_choose_bit_dialog);
		
		HorizontalScrollView horizontalScrollView = new HorizontalScrollView(VtmtTouchScreenChooseParametersActivity.this);
		...
		layout.addView(new TextView(VtmtTouchScreenChooseParametersActivity.this), layoutParams2);
		
		for(int i=from; i<=to; i++){
			TextView textView = new TextView(VtmtTouchScreenChooseParametersActivity.this);
			textView.setText(i+"");
...
//		dialog.onWindowAttributesChanged(wmlLayoutParams);
	...
	private void choosedResponseType(){
		RelativeLayout.LayoutParams params=new RelativeLayout.LayoutParams(RelativeLayout.LayoutParams.WRAP_CONTENT,RelativeLayout.LayoutParams.WRAP_CONTENT);
		params.topMargin=50;
		params.addRule(RelativeLayout.CENTER_HORIZONTAL, HearingConstants.LayoutId.LAYOUT_ID_TWO);
		...
		response=new LinearLayout(this);
		response.setId(HearingConstants.LayoutId.LAYOUT_ID_SEVEN);
		response.setBackgroundDrawable(getResources().getDrawable(R.drawable.ui_params_btn_normal));
		...
		TextView deputyTitle=new TextView(this);
		deputyTitle.setId(HearingConstants.ViewId.VIEW_ID_TWO);
		deputyTitle.setText(responseTypeText);
		deputyTitle.setTextSize(20);
		deputyTitle.setTypeface(null,Typeface.BOLD);
		deputyTitle.setGravity(Gravity.CENTER);
		deputyTitle.setTextColor(R.color.hearing_system_green);
		
		framework.addView(response,params);
		response.addView(title, params2);
		response.addView(deputyTitle, params3);
	}	
	
	private void choosedResponseTypePanel(){
		RelativeLayout.LayoutParams params=new RelativeLayout.LayoutParams(820,RelativeLayout.LayoutParams.WRAP_CONTENT);
		params.topMargin=-6;
		params.addRule(RelativeLayout.CENTER_HORIZONTAL, HearingConstants.LayoutId.LAYOUT_ID_TWO);
		...
		responsePanel=new RelativeLayout(this);
		responsePanel.setId(HearingConstants.LayoutId.LAYOUT_ID_EIGHT);
		responsePanel.setVisibility(RelativeLayout.GONE);
		responsePanel.setBackgroundDrawable(getResources().getDrawable(R.drawable.ui_param_panel_content_backgd));
		
		Button button=new Button(this);
		button.setId(HearingConstants.ViewId.VIEW_ID_ONE);
		button.setText("样本位置出现顺序回应");
		button.setTextSize(16);
		button.setBackgroundDrawable(getResources().getDrawable(R.drawable.cs_choose_params_btn1_1));
		button.setGravity(Gravity.CENTER);
		...
		Button button2=new Button(this);
		button2.setId(HearingConstants.ViewId.VIEW_ID_TWO);
		button2.setText("样本元素出现顺序回应");
		button2.setTextSize(16);
		button2.setBackgroundDrawable(getResources().getDrawable(R.drawable.cs_choose_params_btn1_1));
		button2.setGravity(Gravity.CENTER);
		
		framework.addView(responsePanel,params);
		responsePanel.addView(button, params2);
		responsePanel.addView(button2, params3);
	}
	...
	private void choosedCount(){
		...
		count.setId(HearingConstants.LayoutId.LAYOUT_ID_NINE);
		count.setBackgroundDrawable(getResources().getDrawable(R.drawable.ui_params_btn_normal));
	...
	private void choosedCountPanel(){
		...
		countPanel.setId(HearingConstants.LayoutId.LAYOUT_ID_TEN);
