 ...
 *
 */
public class VHThtrtQuestionAnswerServiceImpl implements HtrtQuestionAnswerService {

	private final static String TAG = "VHThtrtQuestionAnswerServiceImpl";
	...
	public VHThtrtQuestionAnswerServiceImpl() {
		super();
	...
	public HtrtQuestion createQuestion() throws LackOfParametersException, ParameterIsInvalidException {
		...
		if (parameter.getTestType() == 
			...
			answer = getFinger(parameter.getMusic().getSample(), musicInstrumentRedomCode)+"";
			...
			//Log.d(TAG, "key: " + key);
			//手+按键确定按键代码,作为答案
			answer = scaleKeypadMap.get(scaleLevel)+"";
		}else {
			Log.e(TAG, "parameter is null, we stop creating question.");
	...
	private int getFinger(int instrumentType, int musicInstrument){
